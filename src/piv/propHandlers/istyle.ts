import { MayArray, flapDeep, mergeObjects, shakeNil, shrinkFn } from "@edsolater/fnkit"
...
export function parseIStyles<Controller extends ValidController = ValidController>(
  ...
  return mergeObjects(...shakeNil(flapDeep(styles).map((style) => shrinkFn(style, [controller]))))
}
